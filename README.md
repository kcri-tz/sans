...
**New:** Compile parameter *DmaxN* can be set automatically by using the script *SANS-autoN.sh*. This script can be used exactly as the main binary *SANS*. It runs *SANS* with all provided parameters and adds option *-M* to compare *DmaxN* and the actual number of input files.
If SANS has been compiled with a value for *DmaxN* that is neither too small nor much too large, SANS is executed as usual.
If *DmaxN* has been chosen too small or much too large, the script generates a new makefile (*makefile_auton*), re-compiles *SANS* and re-runs *SANS*.
